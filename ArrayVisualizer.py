...
class ArrayVisualizer:
    def __init__(self, array: Array):
        ...
        COLOR_MAPPING.update({"blue": Back.LIGHTBLUE_EX + Style.BRIGHT, "yellow": Back.LIGHTYELLOW_EX + Style.BRIGHT})
    ...
    def visualize_insertion_sort(self):
        ...
        for i in range(1, self.array.size):
            print(f"\nIteration {i}:")
            print(f"Finding element {self.array.get_index_data(i)}'s place in the sorted portion of the array")
            sleep(1)
            ...
            for k in range(self.array.size):
                if k == i:
                    self.print_array_element(self.array.get_index_data(k), "blue")  # element being inserted
                elif k < i:
                    self.print_array_element(self.array.get_index_data(k), "green")  # sorted
                else:
                    self.print_array_element(self.array.get_index_data(k), "dark_green")  # unsorted
                sleep(0.5)
