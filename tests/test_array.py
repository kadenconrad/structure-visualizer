...
# run `pytest tests/test_array.py` from root directory to test
...
class TestArrayInitialization:
    ...
    def test_invalid_size_zero(self):
        with pytest.raises(
            ValueError, match="Array size must be valid positive integer"
        ):
            Array("int", 0)
    ...
    def test_string_array_without_max_length(self):
        with pytest.raises(
            ValueError, match="Must specify string length if declaring string array"
        ):
            Array("str", 5)
