...
# run `pytest tests/test_sorted.py` from root directory to test
...
def test_insertion_sort():
    for i in range(len(TEST_CASES)):
        ...
        assert (
            result_list == SORTED_CASES[i]
        ), f"{TEST_CASES[i]} failed ❌ Expected result: {SORTED_CASES[i]}"
...
def test_binary_search():
    for i in range(len(TEST_CASES)):
        ...
        assert (
            result == idx or -1
        ), f"Expected result: {idx if idx < TEST_CASES[i].size else -1}; returned result: {result}."
